...
const ui = {
    ...
    optAccel: document.getElementById('opt-accel'),
    dispAccel: document.getElementById('disp-accel'),
    optSens: document.getElementById('opt-sens'),
...
const sendConfig = () => {
    if(state.isHost && socket) {
        socket.emit('updateRoomConfig', {
            maxSpeed: parseInt(ui.optMaxSpeed.value),
            accel: parseInt(ui.optAccel.value)
        });
    }
};
...
ui.chkLb.onchange = (e) => { ui.leaderboard.style.display = e.target.checked ? 'block' : 'none'; };
...
document.getElementById('btn-connect').onclick = () => {
    document.getElementById('status').innerText = "ESTABLECIENDO ENLACE...";
...
function setupSocket() {
    ...
    socket.on('roomList', (list) => {
        ...
        if(list.length===0) ui.roomList.innerHTML = '<div style="padding:10px;color:#666;font-style:italic">NO HAY SALAS ACTIVAS</div>';
        list.forEach(r => {
            ui.roomList.innerHTML += `<div class="room-item"><span>${r.id} (${r.players}/8)</span><button class="main-btn secondary" onclick="window.joinRoomId('${r.id}')" style="width:auto;padding:5px 10px;font-size:0.7rem;margin:0;">ENTRAR</button></div>`;
        });
    });
    ...
    socket.on('configUpdated', cfg => {
        ...
        if(!state.isHost) {
            ui.optMaxSpeed.value = cfg.maxSpeed; ui.dispMaxSpeed.innerText = cfg.maxSpeed;
            ui.optAccel.value = cfg.accel; ui.dispAccel.innerText = cfg.accel;
        }
    });
...
function startGame(data) {
    ...
    if(state.isHost) {
        ui.hostControls.classList.remove('disabled-opt');
        ui.adminBadge.style.display = 'inline-block';
    } else {
        ui.hostControls.classList.add('disabled-opt');
...
// ... (GENERACIÓN PROCEDURAL - Copiar clase Chunk, createGridGeometry, noise functions de respuestas anteriores) ...
// Por brevedad, incluyo aquí la clase Chunk resumida, asegúrate de usar la versión completa con líneas visibles.
...
class Chunk {
    constructor(idx, startP, startA, globalDist) {
        this.index = idx; this.startDist = globalDist; this.group = new THREE.Group(); this.clouds = []; scene.add(this.group);
        const angleChange = (rng() - 0.5) * 0.5; const endAngle = startA + angleChange;
        const endX = Math.cos(endAngle) * CONFIG.CHUNK_LENGTH + startP.x; const endZ = Math.sin(endAngle) * CONFIG.CHUNK_LENGTH + startP.z;
        let tH = getTerrainHeight(endX, endZ); let targetY = (tH < 1) ? Math.max(startP.y, 5) : tH + 3.0; targetY = THREE.MathUtils.clamp(targetY, startP.y - 6.0, startP.y + 6.0);
        const p3 = new THREE.Vector3(endX, targetY, endZ);
        const cp1 = new THREE.Vector3(Math.cos(startA)*50, 0, Math.sin(startA)*50).add(startP); const cp2 = new THREE.Vector3(Math.cos(endAngle)*-50, 0, Math.sin(endAngle)*-50).add(p3);
        this.curve = new THREE.CubicBezierCurve3(startP, cp1, cp2, p3);
        this.length = this.curve.getLength(); this.endDist = globalDist + this.length; this.endPoint = p3; this.endAngle = endAngle;
        this.buildGeometry(); this.buildTerrain(); this.buildProps(); this.buildClouds(); this.buildAtmosphere();
    }
    buildGeometry() { /* ... Usar la versión completa del paso anterior para líneas visibles ... */ 
        const div = 40; const pts = this.curve.getSpacedPoints(div); const frames = this.curve.computeFrenetFrames(div, false);
        const rV=[],rI=[],wV=[],wI=[],lV=[],lI=[],yV=[],yI=[]; const lineYOffset = CONFIG.ROAD_Y_OFFSET + 0.15; 
        for(let i=0; i<=div; i++) {
            ...
            const yTop = p.y + CONFIG.ROAD_Y_OFFSET + CONFIG.WALL_HEIGHT; const yBot = p.y - 2.0;
            const L_In = p.clone().add(n.clone().multiplyScalar(CONFIG.ROAD_WIDTH_HALF)); const L_Out = p.clone().add(n.clone().multiplyScalar(CONFIG.ROAD_WIDTH_HALF + CONFIG.WALL_WIDTH));
            wV.push(L_In.x, yTop, L_In.z, L_In.x, yBot, L_In.z, L_Out.x, yTop, L_Out.z, L_Out.x, yBot, L_Out.z);
            const R_In = p.clone().add(n.clone().multiplyScalar(-CONFIG.ROAD_WIDTH_HALF)); const R_Out = p.clone().add(n.clone().multiplyScalar(-(CONFIG.ROAD_WIDTH_HALF + CONFIG.WALL_WIDTH)));
        ...
        const rG = new THREE.BufferGeometry(); rG.setAttribute('position', new THREE.Float32BufferAttribute(rV, 3)); rG.setIndex(rI); rG.computeVertexNormals(); const rM = new THREE.Mesh(rG, matRoad); rM.receiveShadow = true; this.group.add(rM);
        const wG = new THREE.BufferGeometry(); wG.setAttribute('position', new THREE.Float32BufferAttribute(wV, 3)); wG.setIndex(wI); wG.computeVertexNormals(); const wM = new THREE.Mesh(wG, matWall); wM.castShadow = true; wM.receiveShadow = true; this.group.add(wM);
    ...
    buildTerrain() {
        const div=25; const w=400; const divW=10; const vs=[],cs=[]; const cObj=new THREE.Color();
        const pts=this.curve.getSpacedPoints(div); const frames=this.curve.computeFrenetFrames(div, false);
        for(let i=0; i<=div; i++) {
            const P=pts[i]; const N=frames.binormals[i];
            for(let j=0; j<=divW; j++) {
                const u=(j/divW)-0.5; const xOff=u*w; const px=P.x+N.x*xOff; const pz=P.z+N.z*xOff; let py=getTerrainHeight(px, pz);
        ...
        const g=createGridGeometry(vs,cs,div,divW); const m=new THREE.Mesh(g, new THREE.MeshStandardMaterial({vertexColors:true, flatShading:true})); m.receiveShadow=true; this.group.add(m);
        const mid=this.curve.getPointAt(0.5); const wa=new THREE.Mesh(new THREE.PlaneGeometry(300,300), matWater); wa.rotation.x=-Math.PI/2; wa.position.set(mid.x,-2,mid.z); this.group.add(wa);
    }
    buildProps() {
        for(let i=0; i<=1; i+=0.15) {
            const p=this.curve.getPointAt(i); const th=getTerrainHeight(p.x,p.z);
            if(p.y>th+4) { const h=(p.y-0.5)-th; const pil=new THREE.Mesh(new THREE.BoxGeometry(6,h,4), matPillar); pil.position.set(p.x, th+h/2, p.z); pil.castShadow=true; this.group.add(pil); }
        }
        for(let i=0; i<20; i++) {
            ...
            const pos=p.clone().add(bin.multiplyScalar(side*dist)); const y=getTerrainHeight(pos.x,pos.z);
            if(y>0) {
                const gr=new THREE.Group(); const tr=new THREE.Mesh(new THREE.CylinderGeometry(0.5,0.9,10,5), matWood); tr.position.y=5;
                const lv=new THREE.Mesh(new THREE.ConeGeometry(3.5,9,5), matLeaves); lv.position.y=10;
                gr.add(tr,lv); gr.position.set(pos.x, y-2.0, pos.z); gr.scale.setScalar(0.8+rng()*0.5); gr.castShadow=true; this.group.add(gr);
            }
        }
    }
    buildClouds() {
        if(rng()>0.2) {
            const c=new THREE.Group(); const segs=4+Math.floor(rng()*5);
            for(let i=0; i<segs; i++) { const sz=6+rng()*10; const m=new THREE.Mesh(new THREE.DodecahedronGeometry(sz,0), matCloud); m.position.set((rng()-0.5)*20,(rng()-0.5)*8,(rng()-0.5)*20); m.castShadow=true; c.add(m); }
            const p=this.curve.getPointAt(0.5); c.position.set(p.x+(rng()-0.5)*400, 50+rng()*40, p.z+(rng()-0.5)*400);
            this.group.add(c); this.clouds.push(c);
        }
    }
    buildAtmosphere() {
        const pGeo=new THREE.BufferGeometry(); const pPos=[];
        for(let i=0; i<250; i++) { const t=Math.random(); const p=this.curve.getPointAt(t); pPos.push(p.x+(Math.random()-0.5)*120, p.y+Math.random()*40, p.z+(Math.random()-0.5)*120); }
        pGeo.setAttribute('position', new THREE.Float32BufferAttribute(pPos,3));
        this.group.add(new THREE.Points(pGeo, matAtmosphere));
    }
...
function createGridGeometry(verts, colors, rows, cols) {
    ...
    for(let i=0; i<rows; i++) for(let j=0; j<cols; j++) { const a=i*(cols+1)+j; const b=(i+1)*(cols+1)+j; const c=(i+1)*(cols+1)+(j+1); const d=i*(cols+1)+(j+1); idx.push(a,b,d, b,c,d); }
    g.setAttribute('position', new THREE.Float32BufferAttribute(verts,3)); g.setAttribute('color', new THREE.Float32BufferAttribute(colors,3)); g.setIndex(idx); g.computeVertexNormals(); return g;
}
...
function createCar(colorStr) {
    ...
    const bGeo = new THREE.BoxGeometry(2.0, 0.7, 4.2); const body = new THREE.Mesh(bGeo, matBody); body.position.y = 0.6; body.castShadow = true; car.add(body); car.add(createOutline(bGeo, 1.03).translateY(0.6));
    const cGeo = new THREE.BoxGeometry(1.6, 0.5, 2.0); const cab = new THREE.Mesh(cGeo, new THREE.MeshStandardMaterial({color:0x111111})); cab.position.set(0, 1.2, -0.2); car.add(cab); car.add(createOutline(cGeo, 1.03).translateY(1.2).translateZ(-0.2));
    const sGeo = new THREE.BoxGeometry(2.2, 0.1, 0.6); const sp = new THREE.Mesh(sGeo, new THREE.MeshStandardMaterial({color:0x111111})); sp.position.set(0, 1.3, -2.0); car.add(sp);
    const wGeo = new THREE.CylinderGeometry(0.4, 0.4, 0.4, 16).rotateZ(Math.PI/2); const matW = new THREE.MeshStandardMaterial({color:0x222222});
    [[1, 1.3], [-1, 1.3], [1, -1.3], [-1, -1.3]].forEach(p => { const w = new THREE.Mesh(wGeo, matW); w.position.set(p[0], 0.4, p[1]); car.add(w); car.add(createOutline(wGeo, 1.05).translateX(p[0]).translateY(0.4).translateZ(p[1])); });
    const tlGeo = new THREE.BoxGeometry(0.4, 0.2, 0.1); const tl1 = new THREE.Mesh(tlGeo, matTailLight); tl1.position.set(0.6, 0.7, -2.15); car.add(tl1); const tl2 = new THREE.Mesh(tlGeo, matTailLight); tl2.position.set(-0.6, 0.7, -2.15); car.add(tl2); const tlGlow = new THREE.PointLight(0xff0000, 2, 5); tlGlow.position.set(0, 0.7, -2.5); car.add(tlGlow);
    return car;
}
...
function animate() {
    ...
    frames++;
    if(now - lastFpsTime >= 1000) { ui.fps.innerText = "FPS: " + frames; frames=0; lastFpsTime=now; }

    if(state.inGame) {
        // FÍSICAS LOCALES
        const maxSpeed = state.settings.maxSpeed / 100.0; 
        const accel = (state.settings.accel / 100.0) * dt * 2.0;
        
        if(state.input.gas) { if(state.manualSpeed < maxSpeed) state.manualSpeed += accel; } 
        else if(state.input.brake) { state.manualSpeed -= accel * 2.5; } 
        else { state.manualSpeed *= 0.99; }
        ...
        if(curData) {
            ...
            state.trackDist += moveVec.dot(curData.tan);
            state.lateralOffset += moveVec.dot(curData.right);

            if(Math.abs(state.lateralOffset) > CONFIG.WALL_LIMIT) {
                const roadAngle = Math.atan2(curData.tan.x, curData.tan.z);
                let rel = state.worldHeading - roadAngle;
                while(rel > Math.PI) rel -= Math.PI*2; while(rel < -Math.PI) rel += Math.PI*2;
                state.lateralOffset = Math.sign(state.lateralOffset) * CONFIG.WALL_LIMIT;
                state.worldHeading = roadAngle + (-rel * 0.5);
                state.manualSpeed *= 0.6;
            }
        }
        ...
        if(finalData) {
            const pos = finalData.pos.clone();
            pos.add(finalData.right.multiplyScalar(state.lateralOffset));
            pos.y += CONFIG.ROAD_Y_OFFSET + 0.05;
            ...
            camera.lookAt(pos.clone().add(new THREE.Vector3(0, 2, 0)).add(finalData.tan.multiplyScalar(lookOffset)));
        ...
        // ENVIAR ESTADO
        socket.emit('myState', {
            d: parseFloat(state.trackDist.toFixed(2)),
            l: parseFloat(state.lateralOffset.toFixed(2)),
            s: parseFloat(state.manualSpeed.toFixed(3)),
            h: parseFloat(state.worldHeading.toFixed(3))
        });

        // ENTORNO
        const tEnv = now * 0.00005;
        const carPos = mainCar.position;
        sunLight.position.set(carPos.x + Math.cos(tEnv)*1500, Math.sin(tEnv)*1500, carPos.z);
        sunLight.target.position.copy(carPos); moonLight.position.set(carPos.x - Math.cos(tEnv)*1500, -Math.sin(tEnv)*1500, carPos.z);
        chunks.forEach(c => c.clouds.forEach(cl => cl.position.z += 0.2));
        
        const sin = Math.sin(tEnv);
        let target = new THREE.Color();
        let opStars = 0;
        if (sin < -0.4) { target.copy(COLORS.night); opStars = 1; } 
        else if (sin < 0.1) { const t=(sin+0.4)/0.5; target.copy(COLORS.night).lerp(COLORS.dawn, t); opStars=1-t; }
        else if (sin < 0.4) { const t=(sin-0.1)/0.3; target.copy(COLORS.dawn).lerp(COLORS.day, t); opStars=0; }
        else if (sin < 0.8) { target.copy(COLORS.day); opStars=0; }
        else { const t=(sin-0.8)/0.2; target.copy(COLORS.day).lerp(COLORS.dusk, t); opStars=t*0.5; }
        
        scene.background = target; scene.fog.color.copy(target);
        starField.material.opacity = opStars; starField.position.copy(carPos);
        ambientLight.intensity = Math.max(0.2, sin + 0.5);
...
function updateRemotePlayers(data) {
    // 1. ACTUALIZAR POSICIONES REMOTAS
    data.forEach(p => {
        if(p.i === state.myId) return;
        if(!state.players[p.i]) {
            const mesh = createCar(p.c); scene.add(mesh); state.players[p.i] = { mesh: mesh };
        }
        const other = state.players[p.i];
        const tData = getTrackData(p.d);
        if(tData) {
            const pos = tData.pos.clone().add(tData.right.multiplyScalar(p.l));
            pos.y += CONFIG.ROAD_Y_OFFSET + 0.05;
            other.mesh.position.lerp(pos, 0.3);
            
            const curRot = other.mesh.rotation.y;
            let tgtRot = p.h;
            if(tgtRot - curRot > Math.PI) tgtRot -= Math.PI*2;
            if(tgtRot - curRot < -Math.PI) tgtRot += Math.PI*2;
            ...
            if(p.s > 0.3 && Math.random() > 0.7) {
                ...
                s.position.copy(other.mesh.position).add(new THREE.Vector3((Math.random()-0.5), 0.2, -2));
    ...
    // 2. ACTUALIZAR LEADERBOARD
    // Construir lista con todos (yo + remotos)
    const allRacers = [];
    
    // Yo
    allRacers.push({
        name: 'Player ' + getMyLetter(data), // Necesitamos saber mi letra. El servidor la envía en pack para todos.
        dist: state.trackDist,
        isMe: true
    });

    // Remotos (Data contiene a todos excepto a veces si el server hace culling, pero asumimos broadcast total)
    // El servidor envía {i, n, d, ...} donde n es la letra.
    // OJO: El servidor envía mi propio dato también en 'u'. Lo filtramos arriba para no pintar coche fantasma,
    // pero para el leaderboard es útil.
    
    // Vamos a reconstruir la lista desde 'data' que trae todo el estado de la sala
    const lbList = data.map(p => ({
        name: 'PLAYER ' + p.n,
        dist: p.d,
        isMe: p.i === state.myId
    }));
    ...
    // Ordenar descendente
    lbList.sort((a, b) => b.dist - a.dist);

    // Render HTML
    if(lbList.length > 0) {
        const leaderDist = lbList[0].dist;
        let html = '';
        lbList.forEach((p, idx) => {
            let valStr = '';
            if (idx === 0) valStr = (p.dist/1000).toFixed(2) + ' KM';
            else valStr = '-' + ((leaderDist - p.dist)/1000).toFixed(2) + ' KM';
            
            html += `<div class="lb-row ${p.isMe ? 'me' : ''} ${idx===0 ? 'first' : ''}">
                <span class="lb-name">${idx+1}. ${p.name}</span>
                <span>${valStr}</span>
            </div>`;
        });
...
// Helper para encontrar mi letra si no llega directo
function getMyLetter(data) {
    const me = data.find(p => p.i === state.myId);
    return me ? me.n : '?';
}
